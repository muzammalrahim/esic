                ...
                <div class="form">
                    <div class="filter3 row" id="filter">
                        ...
                        <div class="location col-lg-4 col-md-4">
                            <select id="locationSelect" name="locationSelect[]" class="customMultiSelect2 form-control" data-placeholder="Select Location" multiple="multiple" style="width:100%!important;height:50px!important">
                                <?php
                                    if(isset($selectors) and !empty($selectors['states'])){
                                        foreach ($selectors['states'] as $state) {
                                            if(!empty($state)){
                                                ...
                                                echo '<option value="' . $state['Value'] . '" '.$selected.'>' . $state['State'] . '</option>';
                                            }
                                        }
                                    }
                                ?>
...
<section class="listing">
		<div class="container">
            ...
            foreach ($return as $key => $item) {
                ...
                $prior_year_status = '';
                $prior_year_status = $item->prior_year_status;
                $date_updated      =  $item->date_updated;
                $obj               = new DateTime($date_updated);
                $date_updated = $obj->format('Y');
				...
				<div class="list-item" data-item="<?= $key ;?>">
					...
					<div class="item-detail">
                        ...
                        <?php
                        $jsons ='';
                        $jsons = json_decode($prior_year_status ,true);
                        if($jsons){
                            $push = array();
                            $counter=1;
                            foreach($jsons as $json){
                                $array = array();
                                $counter++; // listed 3 years status
                                if(isset($this->data['esic_status']) && !empty($this->data['esic_status'])){
                                    foreach( $this->data['esic_status'] as $esic_status ){
                                        if($json['status'] == $esic_status->id){
                                            $array =array(
                                                'year'   => $json['year'],
                                                'status' => $esic_status->status,
                                                'color' => $esic_status->color
                                            );
                        ...
                        if(($push)){ ?>
                            <div class="item-status ">
                                <span class="label label" style="color:Black;font-size: 100%;" data-toggle="tooltip" title="Prior Year Status" data-placement="bottom">
                                    <?php
                                    if(isset($push) && !empty($push)){
                                        foreach( $push as $esic_status ){?>
                                               Status: <?= $esic_status['year'] ?> <p style="background-color:<?= $esic_status['color'] ?>;color:#ffffff" class="btn btn-xs"> <?= $esic_status['status']; ;?>  </p><br>
                                            <?php
                                        }
                                    }?>
                                </span>
                        ...
                        <?php unset($push);  } else { ?>
                            <div class="item-status ">
                                <span class="label label" style="color:Black;font-size: 100%;" data-toggle="tooltip" title="Prior Year Status" data-placement="bottom">
                                    ...
                                    if(isset($this->data['esic_status']) && !empty($this->data['esic_status'])){
                                        foreach( $this->data['esic_status'] as $esic_status ){
                                           if( $esic_status->id == $item->status ){
                                               ?>
                                               Status: <?= $date_updated ?> <p style="background-color:<?= $esic_status->color ?>;color:#ffffff" class="btn btn-xs"> <?= $esic_status->status; ?> </p>
                                                <?php
                                           }
                                        }
                                    }?>
