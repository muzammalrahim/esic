                    ...
                    <div class="form">
                        <div class="filter3 row" id="filter">
                            <div class="col-lg-4 col-md-4">
                                ...
                                ?>
                                <input type="text" name="searchBox" id="searchbox" class="searchBox ac_input" placeholder="Name, Company, website" autocomplete="off" value="<?=isset($searchBoxValue)?$searchBoxValue:'';?>">
                            </div>
                            <div class="location col-lg-4 col-md-4">
                                <select id="locationSelect" name="locationSelect[]" class="customMultiSelect2 form-control" data-placeholder="Select Location" multiple="multiple">
                                    ...
                                    if(isset($filters['Locations'])){
                                        ...
                                        if(!empty($filters['Locations']['streets'])){
                                            echo '<optgroup label="Street">';
                                            foreach($filters['Locations']['streets'] as $street){
                                                if(isset($postedValues['locationSelect']) and !empty($postedValues['locationSelect'])){
                                                    if(!empty($street->name) and in_array(strtolower($street->name),$postedValues['locationSelect'])){
                                                        $selected = 'selected="selected"';
                                                    }else{
                                                        $selected = '';
                                                    }
                                                ...
                                                if(!empty($street->name)){
                                                    echo '<option value="'.strtolower($street->name).'" '.$selected.' >'.$street->name.'</option>';
                                                }
                                            }
                                            echo '</optgroup>';
                                        }
            ...
            foreach ($return as $key => $item) {
				...
				<div class="list-item" data-item="<?= $key ;?>">
					...
					<div class="item-detail">
						...
						<?php if(isset($item->investorType) && !empty($item->investorType)){ ?>
						<div class="item-type">
						...
						<?php } ?>
						<?php if(isset($item->preferredInvestmentAmount) && !empty($item->preferredInvestmentAmount)){ ?>
						<div class="item-investment">
							<span>
								<?= $item->preferredInvestmentAmount; ?>
							</span>
						</div>
						<?php } ?>
